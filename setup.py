...
try:
    count = (
        run(["git", "rev-list", "--count", "HEAD"], stdout=PIPE, check=True)
        .stdout.splitlines()[0]
        .decode("utf-8")
    )
    VERSION = "%s" % count
...
script_unix = ["scripts/unix/neopo", "scripts/unix/particle"]

script_windows = ["scripts/windows/neopo.cmd", "scripts/windows/particle.cmd"]
