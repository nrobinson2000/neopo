...
from .manifest import create_manifest, get_manifest_value, write_manifest
from .utility import write_executable, write_file
...
def get_extension_url(extension_name=WORKBENCH_EXTENSION):
    ...
    payload = (
        '{"assetTypes":null,"filters":[{"criteria":[{"filterType":7,"value":"%s"}],"direction":2,"pageSize":100,"pageNumber":1,"sortBy":0,"sortOrder":0,"pagingToken":null}],"flags":103}'
        % extension_name
    )
...
def install_or_update(install, force, skip_deps):
    ...
    if install:
        ...
        if install_platform == "aarch64":
            buildtools_version = [
                dep["version"] for dep in dep_json if dep["name"] == "buildtools"
            ][0]
            fix_buildtools(buildtools_version)
            openocd_version = [
                dep["version"] for dep in dep_json if dep["name"] == "openocd"
            ][0]
        ...
        if skipped_deps:
            ...
            print(
                *["%s@%s" % (dep["name"], dep["version"]) for dep in skipped_deps],
                sep=", ",
            )
        print()
...
def attempt_download(firmware):
    try:
        ...
        raise DependencyError(
            "DeviceOS version %s not found!" % firmware["version"]
        ) from error
...
def setup_workbench_arm(extensions, exts):
    ...
    particle_ext = [
        ext for ext in exts if ext.startswith("particle.particle-vscode-core")
    ][0]
...
def setup_debugger_arm(extensions, exts):
    ...
    try:
        node_version = subprocess.run(
            ["node", "-v"], stdout=subprocess.PIPE, check=True
        )
